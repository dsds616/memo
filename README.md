...
Supabase 데이터베이스 기반의 완전한 CRUD 기능을 갖춘 메모 앱으로, 서버 액션과 MCP 연동을 활용한 프로젝트입니다.
...
- **State Management**: React Hooks (useState, useEffect, useMemo, useTransition)
- **Package Manager**: npm
...
이미 `supabase login`과 `supabase link`가 완료된 상태입니다. 마이그레이션 파일은 `supabase/migrations/` 디렉토리에 있습니다.
...
> **참고**: 프로젝트는 이미 `.env.local` 파일이 설정되어 있으며, Supabase MCP를 통해 마이그레이션이 적용되었습니다.
...
├── supabase/
│   ├── migrations/
│   │   └── 20251105065016_create_memos_table.sql  # DB 마이그레이션
...
Supabase 데이터베이스에 6개의 샘플 메모가 미리 등록되어 있습니다:
...
// 메모 수정
const updatedMemo = await updateMemo(id, formData)
